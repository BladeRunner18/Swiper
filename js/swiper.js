...
(function (global, $, factory) {
    ...
    function upDateClass() {
        ...
        if (swiper.params.loop) {
            var realSlides = swiper.realSlides - swiper.params.slidesPerView + 1;
            var realIndex = index - realSlides;
            if (realIndex < 0) {
                realIndex = -realIndex;
                var length = swiper.realSlides;
                realIndex = length - realIndex;
    ...
    function drag() {
        var swiper = this, isDown = false, x, y, newx, translate, index;
    ...
    function touch() {
        var swiper = this, isTouch = false, x, y, newx, translate;
        ...
        swiper.wrapper.get(0).addEventListener('touchmove', function (event) {
            ...
            if (swiper.isAnimating == true) { return false }
            if (isTouch == true) {
                newx = event.targetTouches[0].pageX - x;
                if (swiper.index === 0 || swiper.index === swiper.slideGrid.length - 1) {
                    newx = newx / 2;
                }
                swiper.doTranslate(-(translate - newx), true);
        ...
        create: function () {
            ...
            for (var i = 0; i < swiper.pagination.els.length; i++) {
                swiper.pagination.els[i].on(eventType, function (j) {
                    return function () {
                        if (swiper.params.loop) {
                            swiper.slideTo(swiper.params.slidesPerView - 1 + j);
                        } else {
                            swiper.slideTo(j);
                        }
                    }
